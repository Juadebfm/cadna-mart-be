...
- [x] Rows 62-78 - Stripe, wallet, and pay-on-delivery
  Stripe: `src/payments/stripe.controller.ts` — 7 stub endpoints (`POST /payments/stripe/payment-intent`, `POST /payments/stripe/payment-intent/:id/confirm`, `POST /payments/stripe/setup-intent`, `POST /payments/stripe/refund`, `GET/DELETE /payments/stripe/payment-methods{/:id}`, `POST /webhooks/stripe`). All deferred — international payments decision pending V1. Wallet: new `src/wallet/` module — `GET /wallet`, `GET /wallet/transactions`, `POST /wallet/topup/initialize`, `POST /wallet/debit`, `POST /wallet/credit` (admin), `POST /wallet/transfer` (deferred), `GET /wallet/holds`. PoD: `src/payments/pod.controller.ts` — `GET /payments/pod/eligibility` (static V1 rules: ≤₦50k, Lagos LGAs), `POST /payments/pod/confirm`.
...
- [x] Rows 217-266 - Partners and rewards
  Partners: new `src/partners/` module — `POST /partners/onboard` (Public), `POST /partners/me/kyc`, `POST /partners/me/agreement`, `GET /partners/me/dashboard`, `GET /partners/me/eligible-items{/:id}`, `POST/GET /partners/me/commitments`, `GET /partners/me/commitments/:id{/status}`, `GET /partners/me/settlements`, `POST /partners/me/disputes`, and admin routes: `GET /admin/partners`, `POST /admin/partners/:id/approve`, `GET/POST /admin/partners/eligibility-rules`, `GET /admin/partners/commitments`, `POST /admin/partners/commitments/:id/approve`, `POST /admin/partners/disputes/:id/resolve`, `GET/PATCH /admin/partners/funding-caps`. Rewards: new `src/rewards/` module — `GET /rewards/program` (Public), `GET /rewards/me`, `GET /rewards/me/cashback`, `GET /rewards/me/transactions`, `POST /rewards/cashback/redeem`, `GET /rewards/tiers` (Public), `GET /rewards/campaigns` (Public), `POST /rewards/affiliates/enroll`, `GET /rewards/affiliates/me{/earnings,/referrals}`, `POST /rewards/affiliates/me/payout`, `GET /rewards/sellers/me/rewards`, `GET /rewards/financiers/me/rewards`, and admin CRUD for program branding, tiers, campaigns, config, fraud alerts (14 admin endpoints).
